...
async function fetchDirectoryTree(dirPath: string): Promise<string[]> {
  ...
  try {
    ...
    debug(`Found ${filePaths.length} file(s) in ${dirPath}: ${JSON.stringify(filePaths)}`);
...
async function syncDirectory(
  ...
  if (filePaths.length === 0) {
    console.log(
      `Warning: Directory ${dirConfig.path} is empty. Skipping.`
    );
    return;
  }
  ...
  for (const filePath of filePaths) {
    ...
    const relativePath = filePath.replace(dirConfig.path, dirConfig.destination);
